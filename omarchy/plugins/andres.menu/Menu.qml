...
Item {
  ...
  Timer {
    ...
    onTriggered: initialRowsAnimation.restart()
  }

  NumberAnimation {
    id: initialRowsAnimation
    target: root
    property: "rowReveal"
    from: 0
    to: 1
  ...
  function beginOpen() {
    closeTimer.stop()
    closingRowsAnimation.stop()
    initialRowsDelay.stop()
  ...
  function startClose() {
    ...
    initialRowsAnimation.stop()
    opened = false
    if (reduceMotion || rowReveal <= 0) beginPanelClose()
    else closingRowsAnimation.restart()
  }
  ...
  property int cardWidth: Math.min(root.dmenuActive ? Style.space(root.dmenuWidth) : ((root.activeMenu === "trigger.capture.screenrecord" || root.activeMenu === "style.font") ? Style.space(520) : Style.space(460)), panel.width - Style.gapsOut * 2)
  ...
  function openExistingMenu(initialMenu) {
    ...
    Qt.callLater(function() {
      root.reveal = 1
      if (!root.reduceMotion) initialRowsDelay.restart()
  ...
  PanelWindow {
    ...
    readonly property int drawerTop: Math.max(0, (root.shell && root.shell.bar ? root.shell.bar.barSize : Style.bar.sizeHorizontal) - 1)
    ...
    FrameJoin {
      ...
      frameColor: root.background
      opacity: card.opacity
      transform: Scale { origin.x: rightJoin.width / 2; xScale: -1 }
    }

    Rectangle {
      id: card
      readonly property int contentTopInset: root.contentMargin
      readonly property int contentRightInset: root.contentMargin
      readonly property int contentBottomInset: root.contentMargin
      readonly property int contentLeftInset: root.contentMargin
      ...
      y: panel.effectiveCardTop - Math.round((1 - root.reveal) * height)
      ...
      Column {
        ...
        Rectangle {
          ...
          radius: Math.min(17, height / 2)
          color: "#121212"
          border.width: 1
          border.color: root.filterText ? "#555555" : "#292929"
        ...
        Item {
          ...
          ListView {
            ...
            section.delegate: Item {
              ...
              opacity: root.reduceMotion ? 1 : root.rowReveal
            ...
            delegate: BorderSurface {
              ...
              readonly property real entranceProgress: root.reduceMotion ? 1 : Math.max(0, Math.min(1,
                (root.rowReveal - Math.min(row.index, 6) * 0.065) / 0.58))

              width: ListView.view.width
              height: root.rowHeightForDetail(row.detail)
              radius: root.cornerRadius
              opacity: entranceProgress
              transform: Translate { y: -(1 - row.entranceProgress) * Style.space(8) }
