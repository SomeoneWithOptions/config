...
name: o-front
description: >
  Use when task asks for visually strong landing page, website, app, prototype,
  demo, or game UI. Enforces restrained composition, image-led hierarchy,
  cohesive content structure, tasteful motion, avoids generic cards, weak
  branding, and UI clutter.
---
...
- If the first screen includes a sticky/fixed header, that header counts against the hero. Combined header + hero content must fit within initial viewport at common desktop and mobile sizes.
- When using `100vh`/`100svh` heroes, subtract persistent UI chrome (`calc(100svh - header-height)`) or overlay header instead of stacking it in normal flow.
...
- minimal chrome
- cards only when card is interaction
...
If a panel can become plain layout without losing meaning, remove card treatment.
...
First viewport needs real visual anchor. Decorative texture is not enough.
...
- Do not include prompt language or design commentary in UI.
- Give every section one responsibility: explain, prove, deepen, or convert.

If deleting 30 percent of copy improves page, keep deleting.

## Utility Copy For Product UI

When work is dashboard, app surface, admin tool, or operational workspace, default to utility copy over marketing copy.

- Prioritize orientation, status, and action over promise, mood, or brand voice.
- Start with working surface itself: KPIs, charts, filters, tables, status, or task context. Do not introduce hero section unless user explicitly asks.
- Section headings should say what area is or what user can do there.
- Good: "Selected KPIs", "Plan status", "Search metrics", "Top segments", "Last sync".
- Avoid aspirational hero lines, metaphors, campaign-style language, executive-summary banners on product surfaces unless specifically requested.
- Supporting text should explain scope, behavior, freshness, or decision value in one sentence.
- If a sentence could appear in homepage hero or ad, rewrite until it sounds like product UI.
- If a section does not help someone operate, monitor, or decide, remove it.
- Litmus check: if an operator scans only headings, labels, and numbers, can they understand page immediately?
...
- sticky storytelling
- carousels that advance narrative, not fill space
- menus, drawers, and modal presence effects

Motion rules:

- noticeable in quick recording
- smooth on mobile
- fast and restrained
...
- No more than one accent color unless product already has strong system.
...
- Sections that repeat same mood statement
...
- Would design still feel premium if all decorative shadows were removed?
